    ...
    */
    ...
    public static function getFormattedDateObject($date, $type = 'datetime', $array = true)
    {
        ...
        try {
            ...
            $jDate = Jalalian::fromCarbon($tmp_date);

            if ($type == 'datetime') {
                $dt['datetime'] = $jDate->format('Y-m-d H:i:s');
                $dt['formatted'] = $jDate->format($settings->date_display_format . ' ' . $settings->time_display_format);
            } else {
                $dt['date'] = $jDate->format('Y-m-d');
                $dt['formatted'] = $jDate->format($settings->date_display_format);
            }
